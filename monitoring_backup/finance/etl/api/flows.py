...
from etl.common.loaders.upsert import make_flow_uid, upsert_flows_native
from etl.common.loaders.validation import validate_flows_rows
from etl.utils.dates import last_month_utc_date, today_utc_date
...
def run(
    ...
    *,
    ...
    if conn is None:
        log.error("dsn_db_connection_failed", extra={"step": "flows", "dsn": dsn})
        return 0
    ...
    try:
        ...
        for path in _iter_asset_yaml_files(assets_dir):
            ...
            if not per_account_rows:
                log.info(
                    "flows_empty",
                    extra={"step": "flows", "provider": prov.provider_name},
                )
                continue
            ...
            for i, r in enumerate(per_account_rows):
                ...
                # Required elements
                if (
                    not isinstance(d, date)
                    or not at
                    or not eid
                    or not asset
                    or amt is None
                    or kind not in {"in", "out"}
                ):
                    bad_rows += 1
                    log.warning(
                        "flows_bad_row_detail",
                        extra={
                            "step": "flows",
                            "provider": prov.provider_name,
                            "raw_row": repr(r),
                ...
                origin_ref = txh
                flow_uid = make_flow_uid(
                    prov.provider_name, account_id, origin_ref or "nohash", asset, kind
                )
                tuples.append(
                    (flow_uid, d, account_id, asset, amount, kind, origin_ref)
                )
            ...
            # 6) Validation
            valid_rows, errors = validate_flows_rows(
                tuples, allow_zero=False, max_date=end_date
            )
            for e in errors:
                log.error(
        ...
        log.info("flows_job_done", extra={"step": "flows", "rows": total_rows_written})
...
def main():
    ap = argparse.ArgumentParser(
        description="ETL - Flows Orchestrator (new format only)"
    )
    ap.add_argument(
        "assets_dir", help="Directory with provider YAML files (e.g., finance/assets)"
    )
    ap.add_argument("--start", help="Start date YYYY-MM-DD (inclusive)")
    ap.add_argument("--end", help="End date YYYY-MM-DD (inclusive)")
    ap.add_argument(
        "--dsn", help="PostgreSQL DSN (overrides PG_DSN env var)", default=None
    )
    args = ap.parse_args()
