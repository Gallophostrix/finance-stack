...
from .batching import bulk_upsert_balances, bulk_upsert_flows, bulk_upsert_prices
...
def upsert_prices_eur(
    conn: PGConnection,
    rows: Iterable[Tuple[str, date, str, Decimal]],  # (asset, d, source, price_eur)
    *,
    logger=None,
    chunk_size: int = 500,
) -> int:
    """
    Upsert prices into market.prices_eur.
    """
    log = logger or setup_json_logging()
    with conn:
        with conn.cursor() as cur:
            ...
            if not batch_rows:
                log.info("prices_upsert_noop", extra={"step": "upsert_prices", "rows": 0})
                return 0
            total = bulk_upsert_prices(cur, batch_rows, chunk_size=chunk_size)
    log.info("prices_upsert_done", extra={"step": "upsert_prices", "rows": total})
    return total
