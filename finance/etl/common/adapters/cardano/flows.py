...
def _collect_candidates(
    ...
    for sk in stake_keys:
        r = koios.get("account_txs", params={"_stake_address": sk}, ctx={"stake_tail": sk[-6:]}) or []
        if not isinstance(r, list):
            log.warning(
                "cardano_account_txs_unexpected_shape",
                extra={"job": "etl-api", "step": "flows", "type": type(r).__name__, "stake_tail": sk[-6:]},
            )
            continue
        for x in r:
            h = x.get("tx_hash")
...
def _flows_from_txinfo(
    ...
    for chunk in batch((x["tx_hash"] for x in txs), txinfo_batch):
        ...
        net: Dict[Tuple[str, str, str, Optional[str], Optional[str]], int] = defaultdict(int)
...
def detect_flows(
    ...
    *,
    ...
    try:
        ...
        addr_to_stake, txs = _collect_candidates(koios, stake_keys, start_date, end_date, log)
        if not addr_to_stake or not txs:
            log.info("cardano_flows_done", extra={"job":"etl-api","step":"flows","rows": 0})
            return []

        # Phase 2: compute per-(stake, tx, asset) net and emit rows
        flows = _flows_from_txinfo(koios, txs, addr_to_stake, wl, txinfo_batch, log)

        log.info("cardano_flows_done", extra={"job":"etl-api","step":"flows","rows": len(flows)})
        return flows
