...
CREATE TABLE IF NOT EXISTS core.assets (
  ...
  class         TEXT NOT NULL CHECK (class IN ('crypto','actions','epargne','immo')),
...
CREATE TABLE IF NOT EXISTS core.providers (
  ...
  provider_type  TEXT NOT NULL CHECK (provider_type IN ('blockchain','bank','broker','cex','self','other')),
...
CREATE TABLE IF NOT EXISTS core.accounts (
  ...
  account_type         TEXT NOT NULL CHECK (account_type IN ('address','stake_key','iban','broker_account','property','other')),
  external_identifier  TEXT NOT NULL,                            -- adresse/stake key/IBAN/id broker...
  label                TEXT,                                     -- "Ledger-1", "PEA Amundi", "Livret A ..."
  "group"              TEXT NOT NULL CHECK ("group" IN ('wallet','CEX','PEA','AV','Bank','Immo')),
  is_active            BOOLEAN NOT NULL DEFAULT TRUE,
